...
const createUser = async function (req, res) {
    try {
        ...
        if (isDeleted == true) return res.status(400).send({ status: false, message: "you can't delete a user while creating" })
...
const updateUser = async function (req, res) {
    try {
        ...
        if (findUserId.isDeleted) {
            return res.status(404).send({ status: false, msg: "user already deleted you can't update" })
        }
        ...
        if (isDeleted == true) return res.status(400).send({ status: false, message: "you can't delete a user while updating" })
        ...
        return res.status(200).send({ status: true, message: "user updated successfully", data: updateUserData })
...
let getUser = async function (req, res) {
    try {
        ...
        const user = await userModel.find(getBook)

        if (user.length == 0) return res.status(404).send({ status: false, message: "user Not Found " })
